import { AccountCreateRequest, LoginRequest, SearchUsersRequest, AddNeighborRequest, AddNeighborResponse, accountCreateHandler, loginHandler, searchUsersByUsername, sendAddNeighborRequest, ResponseEnvelope, AcceptNeighborRequestRequest, acceptRequestHandler, RemoveNeighborRequestRequest, removeNeighborRequestHandler, RemoveNeighborMappingRequest, removeNeighborMappingHandler, listNeighbors, listRequestsReceived, listRequestsSent } from './CoveyTownRequestHandlers';
import DatabaseController, { AccountCreateResponse, LoginResponse, NeighborStatus, ListUsersResponse, UserWithRelationship } from '../database/db';
...
describe('CoveyTownRequestHandlers', () => {
  describe('accountCreateHandler()', () => {
    it('creates successfully', async () => {
      ...
      const req: AccountCreateRequest = { username, password };
    ...
    it('searched and found', async () => {
      ...
      if (userSearchingResp.response) {
        ...
        if (searchResp.response && resp.response) {
          ...
          const idOne: string = usersReturned[0]._id;
          const userOne: string = usersReturned[0].username;
          const relationshipStatus: string = usersReturned[0].relationship.status;
          expect(idOne.toString()).toEqual(ans._id);
